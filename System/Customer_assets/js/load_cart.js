...
// Persist checked item keys so Checkout page can show only checked items (even when opened via link/back)
function saveCheckedItemKeysToStorage() {
    const checked = getCheckedCartItems();
    const keys = checked.map(function (item) { return item.item_key; });
    try {
        localStorage.setItem('matarix_checked_item_keys', JSON.stringify(keys));
    } catch (e) {
        console.warn('Could not save checked item keys to localStorage', e);
    }
}
