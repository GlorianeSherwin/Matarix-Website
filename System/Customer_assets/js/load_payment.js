...
async function displayPaymentOrder(order) {
    ...
    // If user came for reupload (proof rejected or reupload param), open the upload modal immediately
    const urlParams = new URLSearchParams(window.location.search);
    const reuploadParam = urlParams.get('reupload') === '1';
    const proofRejected = order.proof_rejected === 1 || order.proof_rejected === true;
    const isGCash = String(paymentMethod || '').toLowerCase() === 'gcash';
    if ((proofRejected || reuploadParam) && isGCash && !isRejected) {
        setTimeout(() => {
            if (typeof showQRPopupForOrder === 'function') {
                showQRPopupForOrder(order.Order_ID);
            }
        }, 500);
    }
