...
try {
    switch ($method) {
        case 'POST':
            ...
            // Optional fields (price_modifier, stock_level, image_path, sku)
            $priceModifier = isset($input['price_modifier']) ? (trim($input['price_modifier']) === '' ? null : (float)$input['price_modifier']) : null;
            $stockLevel = isset($input['stock_level']) ? (trim($input['stock_level']) === '' ? null : (int)$input['stock_level']) : null;
            $imagePath = isset($input['image_path']) ? trim($input['image_path']) : null;
            $sku = isset($input['sku']) ? trim($input['sku']) : null;
            $hasExtras = $pdo->query("SHOW COLUMNS FROM product_variations LIKE 'price_modifier'")->rowCount() > 0;
            if ($hasExtras) {
                $stmt = $pdo->prepare("INSERT INTO product_variations (Product_ID, variation_name, variation_value, price_modifier, stock_level, image_path, sku) VALUES (:product_id, :variation_name, :variation_value, :price_modifier, :stock_level, :image_path, :sku)");
                $result = $stmt->execute([
                    'product_id' => $productId,
                    'variation_name' => $variationName,
                    'variation_value' => $variationValue,
        ...
        case 'PUT':
            ...
            // Update variation (include optional fields if columns exist)
            $priceModifier = isset($input['price_modifier']) ? (trim($input['price_modifier']) === '' ? null : (float)$input['price_modifier']) : null;
            $stockLevel = isset($input['stock_level']) ? (trim($input['stock_level']) === '' ? null : (int)$input['stock_level']) : null;
            $imagePath = isset($input['image_path']) ? trim($input['image_path']) : null;
            $sku = isset($input['sku']) ? trim($input['sku']) : null;
            $hasExtras = $pdo->query("SHOW COLUMNS FROM product_variations LIKE 'price_modifier'")->rowCount() > 0;
            if ($hasExtras) {
                $stmt = $pdo->prepare("UPDATE product_variations SET variation_name = :variation_name, variation_value = :variation_value, price_modifier = :price_modifier, stock_level = :stock_level, image_path = :image_path, sku = :sku WHERE Variation_ID = :variation_id");
                $result = $stmt->execute([
                    'variation_name' => $variationName,
                    'variation_value' => $variationValue,
                    'price_modifier' => $priceModifier,
