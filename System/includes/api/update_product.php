...
// Validate required fields (stock_level optional when product uses variation-level stock)
$requiredFields = ['product_name', 'price', 'minimum_stock', 'description'];
...
if (isset($input['stock_level']) && (string)$input['stock_level'] === '') {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Missing required field: stock_level']);
    exit;
}
...
try {
    ...
    // When stock_level not provided (product uses variation-level stock), keep existing stock and status
    if (isset($input['stock_level']) && $input['stock_level'] !== null && (string)$input['stock_level'] !== '') {
        $stockLevel = (int)$input['stock_level'];
        $stockStatus = $stockLevel <= 0 ? 'Out of Stock' : ($stockLevel < $minimumStock ? 'Low Stock' : 'In Stock');
    } else {
        $curStmt = $pdo->prepare("SELECT stock_level, stock_status FROM products WHERE Product_ID = :product_id LIMIT 1");
        $curStmt->execute(['product_id' => $productId]);
        $cur = $curStmt->fetch(PDO::FETCH_ASSOC);
        $stockLevel = $cur ? (int)$cur['stock_level'] : 0;
        $stockStatus = $cur ? $cur['stock_status'] : 'Out of Stock';
    }
