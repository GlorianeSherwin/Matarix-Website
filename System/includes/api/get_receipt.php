...
try {
    ...
    // Check if delivery_method column exists in orders
    $hasDeliveryMethod = $pdo->query("SHOW COLUMNS FROM orders LIKE 'delivery_method'")->rowCount() > 0;
    $deliveryMethodCol = $hasDeliveryMethod ? 'o.delivery_method,' : '';
    // Check if Discount column exists in transactions (optional; schema may not have it)
    $hasTransactionDiscount = $pdo->query("SHOW COLUMNS FROM transactions LIKE 'Discount'")->rowCount() > 0;
    $discountCol = $hasTransactionDiscount ? 'COALESCE(t.Discount, 0) as Discount,' : '';
    ...
    // Get order items with product details (include transaction_items.variation for selected variation text)
    $hasTiVariation = $pdo->query("SHOW COLUMNS FROM transaction_items LIKE 'variation'")->rowCount() > 0;
    $tiCols = $hasTiVariation ? 'ti.variation as item_variation,' : '';
    $stmt = $pdo->prepare("
        SELECT 
            ti.Item_ID,
            ti.Product_ID,
    ...
    foreach ($items as $item) {
        // Use stored variation text from order (e.g. "Size: 2x4, Length: 8ft") when available
        $variation = '';
        if (!empty($item['item_variation'])) {
            $variation = trim($item['item_variation']);
        }
        if ($variation === '') {
    ...
    // Format dates (guard against null order_date)
    $orderDate = !empty($order['order_date']) ? new DateTime($order['order_date']) : new DateTime();
    ...
    // Compute delivery fee: Total = Subtotal - Discount + DeliveryFee => DeliveryFee = Total - Subtotal + Discount
    $transactionSubtotal = (float)($order['Subtotal'] ?? $subtotal);
    $discount = (float)($order['Discount'] ?? 0);
    $deliveryFee = (float)$total - $transactionSubtotal + $discount;
    $deliveryMethod = $order['delivery_method'] ?? 'Standard Delivery';
    
    // For Pick Up, delivery fee is always 0 and should not be shown
