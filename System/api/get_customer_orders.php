...
try {
    ...
    if ($orderId) {
        ...
        $checkProofRejectedColumn = $pdo->query("SHOW COLUMNS FROM transactions LIKE 'proof_rejected'")->rowCount() > 0;
        $proofRejectedCol = $checkProofRejectedColumn ? ", COALESCE(t.proof_rejected, 0) as proof_rejected" : "";
            ...
            if ($checkDeliveryMethodColumn) {
                $sql = "
                    SELECT 
                        ...
                        t.Transaction_ID,
                        t.Payment_Status,
                        t.proof_of_payment{$proofRejectedCol},
        ...
        // If order is rejected, payment_method should be null. Keep payment_method for Pending Approval so customer can see "Update proof of payment" for GCash orders.
        if ($order['status'] === 'Rejected') {
            $order['payment_method'] = null;
        }
        
        $order['has_proof'] = !empty($order['proof_of_payment']) && trim((string)($order['proof_of_payment'] ?? '')) !== '';
        // Ensure proof_rejected is always present for single-order response (frontend relies on it)
        if (!array_key_exists('proof_rejected', $order)) {
            $order['proof_rejected'] = 0;
        }
