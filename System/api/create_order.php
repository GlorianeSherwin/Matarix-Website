...
$paymentMethod = $data['payment_method'] ?? null; // GCash, Cash on Delivery, or On-Site - selected at checkout
...
$deliveryFeeFromClient = isset($data['delivery_fee']) ? (float)$data['delivery_fee'] : null; // Fee when below minimum (Standard Delivery)
...
foreach ($cartItems as $index => $item) {
    ...
    // Hybrid stock: if variation_id provided and variation has stock_level, validate against variation; else product
    $variationId = isset($item['variation_id']) ? (int)$item['variation_id'] : null;
    if (!$variationId && !empty($item['variations']) && is_array($item['variations'])) {
        foreach ($item['variations'] as $v) {
            if (!empty($v['variation_id'])) { $variationId = (int)$v['variation_id']; break; }
            if (!empty($v['variationId'])) { $variationId = (int)$v['variationId']; break; }
        }
    }
    
    $availableStock = (int)$product['stock_level'];
    if ($variationId) {
        $varStmt = $pdo->prepare("SELECT stock_level FROM product_variations WHERE Variation_ID = :vid AND Product_ID = :pid");
        $varStmt->execute(['vid' => $variationId, 'pid' => $productId]);
        $variation = $varStmt->fetch(PDO::FETCH_ASSOC);
        if ($variation && $variation['stock_level'] !== null && $variation['stock_level'] !== '') {
            $availableStock = (int)$variation['stock_level'];
        }
    }
    
    if ($product['stock_status'] === 'Out of Stock' || $availableStock < $quantity) {
        ob_end_clean();
        http_response_code(400);
        echo json_encode([
            'success' => false,
            'message' => "Insufficient stock for product ID $productId. Available: $availableStock, Requested: $quantity"
        ]);
        exit;
    }
}
...
try {
    ...
    if ($deliveryMethod !== 'Pick Up') {
        ...
        if (!$meetsMinimum) {
            // Allow order but add delivery fee (configurable by admin)
            $premiumFee = (float)($settings['premium_delivery_fee'] ?? 500);
            $deliveryFeeToAdd = ($deliveryFeeFromClient !== null && $deliveryFeeFromClient >= 0) 
                ? $deliveryFeeFromClient 
                : $premiumFee;
            $finalAmount += $deliveryFeeToAdd;
        }
        }
    }
    
    // Order flow: No admin approval needed. Customer selects payment at checkout.
    // ALL orders (Delivery & Pick Up) start in Orders Placed (Waiting Payment) - admin reviews and clicks Prepare
    $validPaymentMethods = ['GCash', 'Cash on Delivery', 'On-Site'];
    ...
    // Create order with payment method if provided
    ...
    if ($checkDeliveryMethodColumn) {
        $stmt = $pdo->prepare("
            ...
            VALUES (:user_id, :status, :payment, :amount, :payment_method, :delivery_method, NOW(), :availability_date, :availability_time)
        ");
        $stmt->execute([
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (:user_id, :status, :payment, :amount, :payment_method, NOW(), :availability_date, :availability_time)
        ");
        $stmt->execute([
            'user_id' => $_SESSION['user_id'],
            'status' => $orderStatus,
    ...
    // Transactions table may have enum('GCash','Cash on Delivery') - On-Site might not be supported
    $transactionPaymentMethod = in_array($paymentMethod, ['GCash', 'Cash on Delivery']) ? $paymentMethod : null;
    if ($checkDiscountColumn) {
        $checkPaymentMethodCol = $pdo->query("SHOW COLUMNS FROM transactions LIKE 'Payment_Method'")->rowCount() > 0;
        if ($checkPaymentMethodCol && $transactionPaymentMethod) {
            $stmt = $pdo->prepare("
                INSERT INTO transactions (Order_ID, Subtotal, Discount, Total, Payment_Method, Payment_Status)
                VALUES (:order_id, :subtotal, :discount, :total, :payment_method, :payment_status)
            ");
            $stmt->execute([
                'order_id' => $orderId,
        ...
        if ($checkPaymentMethodCol && $transactionPaymentMethod) {
            $stmt = $pdo->prepare("
                INSERT INTO transactions (Order_ID, Subtotal, Total, Payment_Method, Payment_Status)
                VALUES (:order_id, :subtotal, :total, :payment_method, :payment_status)
            ");
            $stmt->execute([
                'order_id' => $orderId,
                'subtotal' => $totalAmount,
    ...
    // Create transaction items (include variation text and Variation_ID for hybrid stock)
    $checkVariationColumn = $pdo->query("SHOW COLUMNS FROM transaction_items LIKE 'variation'")->rowCount() > 0;
    $checkVariationIdColumn = $pdo->query("SHOW COLUMNS FROM transaction_items LIKE 'Variation_ID'")->rowCount() > 0;
    foreach ($cartItems as $item) {
        $variationText = null;
        $variationIdForInsert = null;
        if (!empty($item['variation_id'])) {
            $variationIdForInsert = (int)$item['variation_id'];
        } elseif (!empty($item['variations']) && is_array($item['variations'])) {
            foreach ($item['variations'] as $v) {
                if (!empty($v['variation_id'])) { $variationIdForInsert = (int)$v['variation_id']; break; }
                if (!empty($v['variationId'])) { $variationIdForInsert = (int)$v['variationId']; break; }
            }
        }
        if (!empty($item['variations']) && is_array($item['variations'])) {
            $parts = [];
            foreach ($item['variations'] as $name => $data) {
                $val = is_array($data) ? ($data['variation_value'] ?? $data) : $data;
                if ($val) $parts[] = trim($name) . ': ' . trim($val);
            }
            $variationText = !empty($parts) ? implode(', ', $parts) : null;
        }
        $cols = ['Order_ID', 'Product_ID', 'Quantity', 'Price'];
        $placeholders = [':order_id', ':product_id', ':quantity', ':price'];
        $params = ['order_id' => $orderId, 'product_id' => $item['product_id'], 'quantity' => $item['quantity'], 'price' => $item['price']];
        if ($checkVariationColumn && $variationText !== null) {
            $cols[] = 'variation';
            $placeholders[] = ':variation';
            $params['variation'] = $variationText;
        }
        if ($checkVariationIdColumn && $variationIdForInsert) {
            $cols[] = 'Variation_ID';
            $placeholders[] = ':variation_id';
            $params['variation_id'] = $variationIdForInsert;
        }
        $stmt = $pdo->prepare("INSERT INTO transaction_items (" . implode(', ', $cols) . ") VALUES (" . implode(', ', $placeholders) . ")");
        $stmt->execute($params);
    }
