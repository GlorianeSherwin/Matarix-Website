...
try {
    ...
    // Pick Up orders are NOT affected by max orders per day - only Standard Delivery
    $deliveryMethod = isset($_GET['delivery_method']) ? trim($_GET['delivery_method']) : 'Standard Delivery';
    $isPickUp = (strtolower($deliveryMethod) === 'pick up' || $deliveryMethod === 'Pick Up');

    $unavailableDates = [];

    // Skip max-per-day check for Pick Up (no limit for store pickups)
    if (!$isPickUp) {
        $maxPerDay = 0;
        ...
        if ($maxPerDay > 0) {
            // Count only Standard Delivery orders per availability_date (exclude Pick Up and Rejected/Cancelled)
            $checkDeliveryMethod = $pdo->query("SHOW COLUMNS FROM orders LIKE 'delivery_method'")->rowCount() > 0;
            $deliveryFilter = $checkDeliveryMethod
                ? " AND (delivery_method IS NULL OR TRIM(COALESCE(delivery_method,'')) = '' OR delivery_method = 'Standard Delivery') "
                : " ";
            $sql = "
