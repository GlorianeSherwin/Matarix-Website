...
try {
    ...
    // Get product variations (include price_modifier, stock_level, image_path, sku if columns exist)
    $variationCols = 'Variation_ID, variation_name, variation_value';
    try {
        $colCheck = $pdo->query("SHOW COLUMNS FROM product_variations");
        $cols = $colCheck->fetchAll(PDO::FETCH_COLUMN);
        if (in_array('price_modifier', $cols)) $variationCols .= ', price_modifier';
        if (in_array('stock_level', $cols)) $variationCols .= ', stock_level';
        if (in_array('image_path', $cols)) $variationCols .= ', image_path';
        if (in_array('sku', $cols)) $variationCols .= ', sku';
    } catch (Exception $e) { /* use base columns */ }
    $variationsStmt = $pdo->prepare("
        SELECT {$variationCols}
    ...
    foreach ($variations as $variation) {
        ...
        if (isset($variation['price_modifier']) && $variation['price_modifier'] !== null && $variation['price_modifier'] !== '') {
            $v['price_modifier'] = (float)$variation['price_modifier'];
        }
        if (isset($variation['stock_level']) && $variation['stock_level'] !== null && $variation['stock_level'] !== '') {
            $v['stock_level'] = (int)$variation['stock_level'];
        }
        if (!empty($variation['image_path'])) $v['image_path'] = $variation['image_path'];
        if (!empty($variation['sku'])) $v['sku'] = $variation['sku'];
        $groupedVariations[$name][] = $v;
    }
