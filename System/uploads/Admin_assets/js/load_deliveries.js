...
async function completeDelivery(deliveryId, orderId, customerName) {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Switch to completed deliveries tab and reload
            setTimeout(() => {
...
async function createDeliveryCard(delivery) {
    ...
    const allowCancelStatus = roleForStatusRules !== 'Delivery Driver';
    ...
    // Action buttons - All roles use ViewOrderAccept.html for same full order details (APIs enforce driver access to assigned orders only)
    const actionButtons = `
        <a href="ViewOrderAccept.html?order_id=${orderId}" class="order-card-btn view-btn" title="View Order Details" data-delivery-id="${deliveryId}" data-order-id="${orderId}" onclick="event.stopPropagation(); event.stopImmediatePropagation(); return true;">
            <i class="fas fa-eye"></i> View
        </a>
    `;
    
    // Add "Complete Delivery" button for non-delivered deliveries
    if (normalizedStatus !== 'Delivered' && normalizedStatus !== 'Cancelled' && !isBlockedByOrderNotReady) {
        // Escape customer name for use in onclick attribute
        const escapedCustomerName = customerName.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        actionButtons += `
            <button type="button" class="order-card-btn delivered-btn" data-delivery-id="${deliveryId}" data-order-id="${orderId}" data-customer-name="${escapedCustomerName}" title="Complete Delivery">
                <i class="fas fa-check-circle"></i> Complete Delivery
            </button>
        `;
    }
    
    // Add "Cancel Delivery" button for Admin/Store Employee (not for Delivery Drivers)
    // Only show if delivery is not already cancelled or delivered
    if (!isDeliveryDriver && normalizedStatus !== 'Delivered' && normalizedStatus !== 'Cancelled' && allowCancelStatus) {
        const escapedCustomerNameForCancel = customerName.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        actionButtons += `
            <button type="button" class="order-card-btn cancel-delivery-btn" data-delivery-id="${deliveryId}" data-order-id="${orderId}" data-customer-name="${escapedCustomerNameForCancel}" title="Cancel Delivery">
...
async function loadDeliveries() {
    try {
        ...
        if (isDeliveryDriverRole(sessionData.user_role)) {
            const hideIds = ['drivers-tab', 'fleet-tab', 'filter-btn', 'export-btn', 'add-driver-btn', 'add-fleet-btn', 'activeDriversStatCard'];
            hideIds.forEach(id => {
            ...
            if (activeName === 'drivers' || activeName === 'fleet') {
        ...
        if (data.success && data.deliveries) {
            ...
            // 1. "Out for Delivery" - status is "Out for Delivery"
            ...
            if (outForDeliveryContainer) {
                ...
                if (outForDeliveryDeliveries.length === 0) {
                    ...
                    const sorted = [...outForDeliveryDeliveries].sort((a, b) => {
                        const dateA = new Date(a.Updated_At || a.Created_At || a.order_date || 0);
                        const dateB = new Date(b.Updated_At || b.Created_At || b.order_date || 0);
                        return dateB - dateA;
                    });
                    ...
                    outForDeliveryContainer.innerHTML = cardHtmls.filter(html => html && html.trim()).join('');
                }
            }
            ...
            if (completedContainer) {
                ...
                if (completedDeliveries.length === 0) {
                    ...
                    const cardHtmls = await Promise.all(cardPromises);
                    completedContainer.innerHTML = cardHtmls.filter(html => html && html.trim()).join('');
                }
            }
...
function initializeDeliveries() {
    ...
    setTimeout(() => {
        ...
        if (driversTab && !isDeliveryDriverRole()) {
        ...
        if (!isDeliveryDriverRole() && document.querySelector('#drivers-tab.active')) {
            loadDrivers();
        }
        ...
        window.deliveriesRefreshInterval = setInterval(() => {
            ...
            if (activeTabName === 'out-for-delivery' || activeTabName === 'completed-delivery') {
                loadDeliveries();
            } else if (activeTabName === 'drivers' && !isDeliveryDriverRole()) {
                // Refresh drivers tab
...
// Re-export to ensure they're available
window.completeDelivery = completeDelivery;
window.showCompleteDeliveryConfirmation = showCompleteDeliveryConfirmation;
