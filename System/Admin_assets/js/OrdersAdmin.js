...
// Search functionality - filter order cards by order number, customer name, amount, date, etc.
function initializeSearch() {
    ...
    function runSearch() {
        ...
        const groups = document.querySelectorAll('.order-date-group');
        ...
        cards.forEach(function(card) {
            var orderId = (card.getAttribute('data-order-id') || '').toString();
            var orderIdEl = card.querySelector('.order-id');
            var customerEl = card.querySelector('.customer-name');
            var amountEl = card.querySelector('.order-amount');
            var metaEl = card.querySelector('.order-meta');
            var ordNum = orderIdEl ? orderIdEl.textContent : '';
            var customer = customerEl ? customerEl.textContent : '';
            var amount = amountEl ? amountEl.textContent : '';
            var meta = metaEl ? metaEl.textContent : '';
            var searchable = [orderId, ordNum, customer, amount, meta].join(' ').toLowerCase();
            var show = searchable.indexOf(searchTerm) !== -1;
            card.style.display = show ? '' : 'none';
        });

        groups.forEach(function(grp) {
            var groupCards = grp.querySelectorAll('.order-card');
            var visibleCount = 0;
            for (var i = 0; i < groupCards.length; i++) {
                if (groupCards[i].style.display !== 'none') visibleCount++;
            }
    ...
    // Allow load_orders.js to re-apply search after orders are refreshed
    window.reapplyOrdersSearch = runSearch;
}
...
function populateOrderSettingsForm(settings) {
    ...
    // Minimum advance notice days (0 = same-day delivery allowed)
    const minAdvanceNotice = document.getElementById('minAdvanceNoticeDays');
    if (minAdvanceNotice) {
        const rawSettings = settings.raw_settings || {};
        const raw = rawSettings.min_advance_notice_days ?? settings.min_advance_notice_days;
        minAdvanceNotice.value = (raw !== undefined && raw !== null && raw !== '') ? raw : 3;
    }
...
async function saveOrderSettings() {
    ...
    const formData = {
        ...
        min_advance_notice_days: (() => { const v = parseInt(document.getElementById('minAdvanceNoticeDays').value, 10); return isNaN(v) ? 3 : v; })(),
