...
function displayOrderDetails(order) {
    ...
    // Update payment method in Order & delivery section (always show row; use — when none)
    ...
    const norm = (v) => (v && typeof v === 'string' && v.trim() !== '' && v !== 'null' && v !== 'NULL' ? v.trim() : null);
    let displayPaymentMethod = norm(orderPaymentMethod) || norm(transactionPaymentMethod) || null;

    if (paymentMethodDetailEl && paymentMethodTextEl) {
        paymentMethodDetailEl.style.display = 'block';
        const pmLower = (displayPaymentMethod && String(displayPaymentMethod).toLowerCase()) || '';
        const isCOD = pmLower.includes('cash on delivery') || pmLower === 'cod';
        if (displayPaymentMethod) {
            paymentMethodTextEl.textContent = '';
            paymentMethodTextEl.appendChild(document.createTextNode(displayPaymentMethod));
            if (isCOD) {
                const codTag = document.createElement('span');
                codTag.className = 'order-payment-tag payment-tag-cod';
                codTag.setAttribute('style', 'margin-left: 8px; vertical-align: middle; font-size: 11px; font-weight: 600; padding: 3px 8px; border-radius: 6px; text-transform: uppercase; letter-spacing: 0.3px; background: #e8f5e9; color: #2e7d32;');
                codTag.textContent = 'COD';
                paymentMethodTextEl.appendChild(codTag);
            }
        } else {
            paymentMethodTextEl.textContent = '—';
        }
        const icon = paymentMethodDetailEl.querySelector('i');
        if (icon && displayPaymentMethod) {
            const pm = (displayPaymentMethod && String(displayPaymentMethod).toLowerCase()) || '';
            if (pm === 'gcash') {
                icon.className = 'fas fa-mobile-alt mr-2';
            } else if (pm === 'on-site' || pm === 'on site' || pm.includes('cash on delivery') || pm === 'cod') {
    ...
    if (deliveryDateEl) {
        if (order.availability_date) {
            const dateStr = formatDeliveryOrPickupDate(order.availability_date);
    ...
    // Assigned driver and vehicle card (only show when there is an assigned driver)
    const driverCardWrap = document.getElementById('assignedDriverVehicleCardWrap');
    const hasAssignedDriver = tracking && tracking.driver_name && String(tracking.driver_name).trim() !== '' && tracking.driver_name !== '—';
    if (driverCardWrap) {
        if (hasAssignedDriver) {
            const driverNameEl = document.getElementById('assignedDriverName');
            const driverPhoneEl = document.getElementById('assignedDriverPhone');
            const vehicleModelEl = document.getElementById('assignedVehicleModel');
            const deliveryTimeEl = document.getElementById('assignedDeliveryTime');
            if (driverNameEl) driverNameEl.textContent = tracking.driver_name || '—';
            if (driverPhoneEl) driverPhoneEl.textContent = tracking.driver_phone || '—';
            if (vehicleModelEl) vehicleModelEl.textContent = tracking.vehicle_model || '—';
            if (deliveryTimeEl) deliveryTimeEl.textContent = tracking.delivery_time_window || '—';
            driverCardWrap.style.display = '';
        } else {
            driverCardWrap.style.display = 'none';
        }
    }
    ...
    if (paymentInfoSection) {
        if (isRejected) {
            ...
            // Show "View proof of payment" link - opens proof image in modal
            const viewProofLink = document.getElementById('viewProofOfPaymentLink');
            if (viewProofLink) {
                viewProofLink.style.display = 'inline-block';
                const proofUpdatedAt = order.proof_updated_at || null;
                const hasProofUpdatedAt = proofUpdatedAt != null && String(proofUpdatedAt).trim() !== '';
                // Show "New proof uploaded" only for re-upload after rejection (proof_updated_at set by backend)
                const showRedTag = hasProof && hasProofUpdatedAt;
                viewProofLink.innerHTML = showRedTag
                    ? '<i class="fas fa-image mr-2"></i>View proof of payment <span class="badge ml-1" style="font-size: 0.65rem; background: #6c757d; color: #fff;">New proof uploaded</span>'
                    : '<i class="fas fa-image mr-2"></i>View proof of payment';
                viewProofLink.onclick = function(e) {
                    e.preventDefault();
                    const baseUrl = getAppBaseUrl();
                    const orderIdFromUrl = new URLSearchParams(window.location.search).get('order_id') || order.Order_ID;
                    const apiUrl = `${baseUrl}api/get_proof_image.php?order_id=${orderIdFromUrl}&t=${Date.now()}`;
                    const modalImage = document.getElementById('proofOfPaymentModalImage');
                    const requestNewProofBtn = document.getElementById('requestNewProofBtn');
                    if (modalImage) {
                        modalImage.src = apiUrl;
                    }
                    if (requestNewProofBtn) {
                        const role = (sessionStorage.getItem('user_role') || window.__MATARIX_USER_ROLE || '').trim();
                        const canReject = (role === 'Admin' || role === 'Store Employee') && isOrderPlaced;
                        requestNewProofBtn.style.display = canReject ? 'inline-block' : 'none';
                    }
    ...
    if (viewReceiptLink) {
        ...
        if (isApproved) {
            ...
            // Add click handler to navigate to receipt page
    ...
    if (isReady && isPickUp) {
        ...
        if (orderCompleteBtn) {
            ...
            orderCompleteBtn.onclick = async function() {
                ...
                let confirmMsg = 'Mark this order as complete? Customer has picked up the order.';
                ...
                if (isCashOnReceipt && order.amount != null && parseFloat(order.amount) > 0) {
                    const formatted = '₱' + parseFloat(order.amount).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                    confirmMsg += '\n\nPlease collect cash: ' + formatted;
                }
                const confirmed = window.AdminNotifications
                    ? await AdminNotifications.confirm(confirmMsg, { title: 'Order Complete', confirmText: 'Complete', cancelText: 'Cancel' })
                    : confirm(confirmMsg);
                if (!confirmed) return;
                try {
...
async function handleDriverVehicleAssignmentForReady(orderId, driverId, vehicleId) {
    ...
    try {
        ...
        const errMsg = error.message || 'Failed to assign driver and vehicle. Please try again.';
        assignError.textContent = errMsg;
        assignError.style.display = 'block';
        if (window.AdminNotifications) {
            AdminNotifications.error(errMsg, { title: 'Error', duration: 6000 });
            const notifications = document.querySelectorAll('.admin-notification.info');
            notifications.forEach(n => {
                const closeBtn = n.querySelector('.admin-notification-close');
                if (closeBtn) closeBtn.click();
...
async function rejectProofOfPayment(orderId, rejectionReason) {
    ...
    const confirmBtn = document.getElementById('rejectProofConfirmBtn');
