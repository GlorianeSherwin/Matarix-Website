...
(function() {
    ...
    function applyAdminRbacGuards(userRole) {
        ...
        const onViewDelivery = path.includes('/admin/viewdelivery.html');
        ...
        const onViewReceipt = path.includes('/admin/viewreceipt.html');

        if (isDriver) {
            // Drivers: deliveries, view delivery details, view receipt, view order details (same page as admin; APIs enforce assigned-only access)
            if (!(onDeliveries || onViewDelivery || onProfile || onAbout || onViewOrderAccept || onViewOrdersReject || onViewReceipt)) {
