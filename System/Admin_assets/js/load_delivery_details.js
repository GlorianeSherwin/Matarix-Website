...
async function loadDeliveryDetails() {
    ...
    try {
        ...
        const availDate = formatDeliveryOrPickupDate(d.availability_date);
        const availTime = formatTime(d.availability_time);
        document.getElementById('availabilityDate').textContent = (availDate || 'N/A') + (availTime ? ' ' + availTime : '');
        ...
        if (viewReceiptLink) {
            viewReceiptLink.href = '../Admin/ViewReceipt.html?order_id=' + (d.Order_ID || '');
        ...
        if (isOutForDelivery) {
            ...
            // Cancel Delivery option removed for admin and employee
