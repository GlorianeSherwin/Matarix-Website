...
try {
    ...
    $driverWhere = '';
    if ($isDriver) {
        // Delivery Driver: include deliveries where they are assigned either via the legacy Driver_ID
        // OR via the multi-driver junction table (if enabled).
        $driverWhere = " AND (d.Driver_ID = :driver_id_main ";
        if ($hasDeliveryDriversJunction) {
            $driverWhere .= " OR EXISTS (
    ...
    $sql = "
        ...
        WHERE LOWER(TRIM(COALESCE(o.status, ''))) NOT IN ('cancelled', 'deleted', 'pending approval', 'rejected') -- Exclude cancelled, deleted, pending approval, and rejected orders
