...
async function createOrderCard(order) {
    try {
        ...
        // Get payment method for display tag
        const paymentMethod = (order.payment_method && 
            order.payment_method !== 'null' && order.payment_method !== 'NULL' && 
            typeof order.payment_method === 'string' && order.payment_method.trim() !== '') 
            ? order.payment_method.trim() : null;
        // Map to display label for tag
        const paymentMethodLabel = paymentMethod === 'Cash on Delivery' ? 'COD' : 
            paymentMethod === 'On-Site' ? 'Pay on Site' : 
            paymentMethod === 'GCash' ? 'GCash' : null;
    ...
    const actionButtons = `
        ...
        const showProofUpdatedTag = proofUpdatedAt && String(proofUpdatedAt).trim() !== '';
        const proofUpdatedTagHtml = showProofUpdatedTag 
            ? `<span class="order-payment-tag payment-tag-proof-updated" style="background: #17a2b8; color: white; font-size: 0.75rem;">Proof of payment updated</span>` : '';
...
async function completePickupOrder(orderId) {
    ...
    const confirmed = await AdminNotifications.confirm(
        'Mark this order as complete? Customer has picked up the order.',
        {
