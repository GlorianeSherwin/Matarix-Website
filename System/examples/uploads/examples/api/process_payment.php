...
try {
    ...
    // On-Site (Pick Up): status = 'Waiting Payment', payment = 'To Pay' - pay when picking up
    // Cash on Delivery (Standard Delivery): status = 'Waiting Payment', payment = 'To Pay' - pay when delivered
    // GCash (with proof, first upload): status = 'Processing', payment = 'Paid'
    // GCash (with proof, reupload): status = 'Waiting Payment', payment = 'To Pay' (admin needs to verify new proof)
    // GCash (without proof): status = 'Waiting Payment', payment = 'To Pay'
    if ($paymentMethod === 'On-Site' || $paymentMethod === 'Cash on Delivery') {
        $newOrderStatus = 'Waiting Payment';
        $newPaymentStatus = 'To Pay';
    } elseif ($paymentMethod === 'GCash' && !empty($proofOfPayment)) {
        if ($isReupload) {
            // Reupload: Set to Waiting Payment so admin can verify the new proof
            $newOrderStatus = 'Waiting Payment';
            $newPaymentStatus = 'To Pay';
        } else {
            // First upload: Set to Processing and Paid
    ...
    // Don't decrease stock on reupload - wait for admin verification
